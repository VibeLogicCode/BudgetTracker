...
--
-- Loan direction (spec docs/superpowers/specs/2026-08-28-loans-lent-direction-design.md, item BU,
-- v1.14.0).
--
-- ADDITIVE ONLY. One ALTER TABLE ADD COLUMN. There is NO table rebuild in this file.
--
...
--
-- THE ADDED CHECK IS FORWARD-ONLY. SQLite's ALTER TABLE ADD COLUMN does not re-validate existing
-- rows against a CHECK added that way -- the same fact drizzle/0007_loans.sql's and
-- drizzle/0013_household_scope.sql's headers record. Harmless here precisely because this is a NEW
-- column: every pre-existing row takes the DEFAULT, which satisfies the CHECK by construction. The
-- cross-table rule that only a kind='loan' item may carry 'lent' is NOT attempted in SQL at all --
-- a CHECK on warranty_items cannot see across to warranty_item_types.kind -- it lives in
-- src/lib/warranty/items.ts beside assertLoanFieldsMatchKind (planner ruling P3).
--
-- Objects that exist ONLY in SQL and have NO Drizzle representation now number, after this
-- migration (entries 1-39 are restated verbatim from drizzle/0013_household_scope.sql's header):
