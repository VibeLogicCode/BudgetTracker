...
function rangeClauses(range: DateRange, scope: PersonScope): SQL[] {
  const clauses: SQL[] = [
    ...
    // Transfers are excluded from every report series, and so is a loan-principal movement
    // (item 8a) -- see NOT_PRINCIPAL_MOVEMENT's own docblock (@/lib/spend-where) for exactly
    // which of the four loan movements that is, and which one (repaying a debt we owe) it
