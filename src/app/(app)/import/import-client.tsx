...
export function ImportClient({
  ...
  /*
    ...
    (`detected.account?.id ?? 0`) applied to a detection that arrived from the batch screen instead
    of from an upload. A file the detector could not place must BLANK the picker, never fall back
    to accounts[0]: a pre-armed wrong answer is worse than an empty one, because dedup is scoped
    per account and a mis-aimed import writes a second copy of every row rather than merging. That
    is the v1.37.0 fix, and seeding from accounts[0] here would have quietly undone it -- caught by
    the auto-preview test, which found an unplaceable file previewing against a guess.
  */
  const [accountId, setAccountId] = useState<number>(
    initialDetection === null ? (accounts[0]?.id ?? 0) : (initialDetection.account?.id ?? 0),
  );
  const [profileId, setProfileId] = useState<number>(
    initialDetection === null
      ? resolveOfferedProfileId(accounts[0]?.importProfileId, profiles)
      : (initialDetection.profile?.id ?? 0),
  );
   ...
   * pickers already resolved -- so every input the preview hop needs is in hand before this
   * component renders. Waiting to be asked meant landing on step 1 behind an empty drop zone and
   * pressing Preview to see rows that could have been fetched already. Clicking a row IS the ask.
   *
   * Only when both ids resolved. A detection that could not tell leaves one of them 0, and the
   * preview route refuses that -- correctly -- so those rows still land on the full wizard, which
   * is the screen that can actually fix them.
   *
   * The ref is what keeps this to ONCE. The effect's own work sets `preview`, and re-running on
   * every dependency change would re-post the file each time a picker moved.
   */
  const autoPreviewed = useRef(false);
  useEffect(() => {
    if (autoPreviewed.current) return;
    if (initialDetection === null || accountId === 0 || profileId === 0) return;
    autoPreviewed.current = true;
    void upload(new FormData());
    // eslint-disable-next-line react-hooks/exhaustive-deps -- once, on mount, by design (see above)
  }, []);
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-5">
      ...
      {accounts.length === 0 ? (
        ...
        <Card>
          <CardHeader
            title={<StepTitle n={1} state="active">{initialDetection === null ? 'Choose a file' : 'This file'}</StepTitle>}
            description={
              initialDetection === null
                ? 'Drop the statement in. The account and the profile are set from what is in the file — change either before you preview.'
                : 'The account and the profile were set from what is in the file. Change either and the rows below are re-read.'
            }
          />
          <CardBody>
              ...
              */}
              {initialDetection === null ? (
                ...
                <p className="text-sm text-ink">
                  <span className="font-medium">{initialDetection.filename}</span>
                  <span className="text-muted"> — already read. Change either picker below to re-read it.</span>
                </p>
              )}
