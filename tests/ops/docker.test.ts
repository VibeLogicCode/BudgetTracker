...
describe('version and changelog', () => {
  ...
  it('MUST-7.1: the 1.48.0 release', () => {
    const pkg = JSON.parse(read('package.json')) as { version: string };
    expect(pkg.version).toBe('1.48.0');
    const changelog = read('CHANGELOG.md');
    expect(changelog).toMatch(/^## \[1\.48\.0\] - 2026-09-19$/m);
    expect(changelog.indexOf('## Unreleased')).toBeLessThan(changelog.indexOf('## [1.48.0]'));
    expect(changelog.indexOf('## [1.48.0]')).toBeLessThan(changelog.indexOf('## [1.47.0]'));
    const current = changelog.slice(changelog.indexOf('## [1.48.0]'), changelog.indexOf('## [1.47.0]'));
    expect(current).toMatch(/ledger/i);
    expect(current).toMatch(/Adjustment/);
    // The release MOVES a balance, which is the one thing an update note must not bury.
    expect(current).toMatch(/higher balance/i);
