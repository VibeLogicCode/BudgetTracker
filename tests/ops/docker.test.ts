...
describe('version and changelog', () => {
  ...
  it('MUST-7.1: the 1.50.1 release', () => {
    const pkg = JSON.parse(read('package.json')) as { version: string };
    expect(pkg.version).toBe('1.50.1');
    const changelog = read('CHANGELOG.md');
    expect(changelog).toMatch(/^## \[1\.50\.1\] - 2026-09-19$/m);
    expect(changelog.indexOf('## Unreleased')).toBeLessThan(changelog.indexOf('## [1.50.1]'));
    expect(changelog.indexOf('## [1.50.1]')).toBeLessThan(changelog.indexOf('## [1.50.0]'));
  });

  it('MUST-7.1: the 1.50.0 release is still recorded intact (append-only discipline)', () => {
    const changelog = read('CHANGELOG.md');
  ...
  it('MUST-7.1: the 1.49.0 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(current).toMatch(/may move again/i);
    expect(current).toMatch(/### Security/);
    expect(current).toMatch(/Loans & Coverage/);
  });

  it('MUST-7.1: the 1.48.0 release is still recorded intact (append-only discipline)', () => {
    const changelog = read('CHANGELOG.md');
    expect(changelog).toMatch(/^## \[1\.48\.0\] - 2026-09-19$/m);
    const section = changelog.slice(changelog.indexOf('## [1.48.0]'), changelog.indexOf('## [1.47.0]'));
    expect(section).toMatch(/ledger/i);
    expect(section).toMatch(/Adjustment/);
    expect(section).toMatch(/higher balance/i);
  });
  ...
  it('MUST-7.1: the 1.47.0 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(section).toMatch(/Interest on loans/i);
    expect(section).toMatch(/Reconcile to a statement/i);
  });
  ...
  it('MUST-7.1: the 1.46.0 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.46.0]')).toBeLessThan(changelog.indexOf('## [1.45.0]'));
    const current = changelog.slice(changelog.indexOf('## [1.46.0]'), changelog.indexOf('## [1.45.0]'));
    expect(current).toMatch(/no longer commits you to it/i);
    expect(current).toMatch(/History is behind a button/i);
    expect(current).toMatch(/no longer quote the person who reported/i);
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.45.0 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.45.0]')).toBeLessThan(changelog.indexOf('## [1.44.1]'));
    const current = changelog.slice(changelog.indexOf('## [1.45.0]'), changelog.indexOf('## [1.44.1]'));
    expect(current).toMatch(/straight to its rows/i);
    expect(current).toMatch(/what each file will actually import/i);
    expect(current).toMatch(/History is back on the import page/i);
    expect(current).toMatch(/account already filled in/i);
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.44.1 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.44.1]')).toBeLessThan(changelog.indexOf('## [1.44.0]'));
    const current = changelog.slice(changelog.indexOf('## [1.44.1]'), changelog.indexOf('## [1.44.0]'));
    expect(current).toMatch(/looked for its words in the wrong place/i);
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.44.0 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.44.0]')).toBeLessThan(changelog.indexOf('## [1.43.0]'));
    const current = changelog.slice(changelog.indexOf('## [1.44.0]'), changelog.indexOf('## [1.43.0]'));
    expect(current).toMatch(/Drop every statement at once/i);
    expect(current).toMatch(/already imported says so up front/i);
    expect(current).toMatch(/Zip files are opened for you/i);
    expect(current).toMatch(/filename used by two accounts/i);
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.43.0 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.43.0]')).toBeLessThan(changelog.indexOf('## [1.42.1]'));
    const current = changelog.slice(changelog.indexOf('## [1.43.0]'), changelog.indexOf('## [1.42.1]'));
    expect(current).toMatch(/how far over/i);
    expect(current).toMatch(/how the categories are doing/i);
    expect(current).toMatch(/blank cell/i);
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.42.1 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.42.1]')).toBeLessThan(changelog.indexOf('## [1.42.0]'));
    const current = changelog.slice(changelog.indexOf('## [1.42.1]'), changelog.indexOf('## [1.42.0]'));
    expect(current).toMatch(/reported itself as found/i);
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.42.0 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.42.0]')).toBeLessThan(changelog.indexOf('## [1.41.2]'));
    const current = changelog.slice(changelog.indexOf('## [1.42.0]'), changelog.indexOf('## [1.41.2]'));
    expect(current).toMatch(/Charts load when you reach them/i);
    expect(current).toMatch(/show something while they load/i);
    expect(current).toMatch(/One way to build a button/i);
    expect(current).toMatch(/text size in the app has a name/i);
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.41.2 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.41.2]')).toBeLessThan(changelog.indexOf('## [1.41.1]'));
    const current = changelog.slice(changelog.indexOf('## [1.41.2]'), changelog.indexOf('## [1.41.1]'));
    expect(current).toMatch(/links a payment to a loan/i);
    expect(current).toMatch(/how broad it is/i);
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.41.1 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.41.1]')).toBeLessThan(changelog.indexOf('## [1.41.0]'));
    const current = changelog.slice(changelog.indexOf('## [1.41.1]'), changelog.indexOf('## [1.41.0]'));
    expect(current).toMatch(/rule that actually decided the row/i);
    expect(current).toMatch(/nowhere near the expected amount/i);
    expect(current).toMatch(/negative month/i);
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.41.0 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.41.0]')).toBeLessThan(changelog.indexOf('## [1.40.0]'));
    const current = changelog.slice(changelog.indexOf('## [1.41.0]'), changelog.indexOf('## [1.40.0]'));
    expect(current).toMatch(/two columns/i);
    expect(current).toMatch(/column is the ranking/i);
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.40.0 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.40.0]')).toBeLessThan(changelog.indexOf('## [1.39.1]'));
    const current = changelog.slice(changelog.indexOf('## [1.40.0]'), changelog.indexOf('## [1.39.1]'));
    // The design pass, in the terms a household would recognise on screen.
    expect(current).toMatch(/nav rail shows the sequence/i);
    expect(current).toMatch(/clipped inside dialogs/i);
    expect(current).toMatch(/contrast failures/i);
    expect(current).toMatch(/Reduced motion/i);
    // A release with NO migration says so.
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.39.1 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.39.1]')).toBeLessThan(changelog.indexOf('## [1.39.0]'));
    const current = changelog.slice(changelog.indexOf('## [1.39.1]'), changelog.indexOf('## [1.39.0]'));
    // The a report this release answers, in their own terms: "price from insurer can change
    // after a year or months".
    expect(current).toMatch(/one-sided/i);
    expect(current).toMatch(/split point/i);
    // A release with NO migration says so, where a release with one names it.
    expect(current).toMatch(/No migration\./);
  });
  ...
  it('MUST-7.1: the 1.39.0 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.39.0]')).toBeLessThan(changelog.indexOf('## [1.38.0]'));
    const current = changelog.slice(changelog.indexOf('## [1.39.0]'), changelog.indexOf('## [1.38.0]'));
    // The a reports this release answers, in their own terms.
    expect(current).toMatch(/A rule can be about an amount/i);
    expect(current).toMatch(/A rule can name a person/i);
    expect(current).toMatch(/Create a rule/i);
    // The two UI defects reported against v1.38.0 itself.
    expect(current).toMatch(/said nothing and left its editor open/i);
    expect(current).toMatch(/grey "Choose File" widget/i);
    // NO new migration: 0024 shipped in 1.38.0 and this is the release that uses it. Said in the
    // section itself, where every other release either names its migration or says there is none.
    expect(current).toMatch(/Migration 0024 shipped in v1\.38\.0/);
  });
  ...
  it('MUST-7.1: the 1.38.0 release is still recorded intact (append-only discipline)', () => {
    ...
    const current = changelog.slice(changelog.indexOf('## [1.38.0]'), changelog.indexOf('## [1.37.0]'));
    expect(current).toMatch(/reads the file the moment you choose it/i);
    expect(current).toMatch(/Assign a transaction to a bill/i);
    expect(current).toMatch(/can be deleted/i);
    expect(current).toMatch(/Migration 0024/);
  });
  ...
  it('MUST-7.1: the 1.37.0 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.37.0]')).toBeLessThan(changelog.indexOf('## [1.36.1]'));
    const current = changelog.slice(changelog.indexOf('## [1.37.0]'), changelog.indexOf('## [1.36.1]'));
    // A real report, so the symptom is recognisable if the pager ever goes missing again.
    expect(current).toMatch(/Previous page \/ Next page/);
    expect(current).toMatch(/Page 1 of 6/);
    // The import half: detection reads the file before the pickers are set.
    expect(current).toMatch(/set from the file itself/i);
    expect(current).toMatch(/Drop a statement/i);
    // Said plainly, because it is the reason detection refuses to guess.
    expect(current).toMatch(/second copy of every row/i);
  ...
  it('MUST-7.1: the 1.36.1 release is still recorded intact (append-only discipline)', () => {
    ...
    expect(changelog.indexOf('## [1.36.1]')).toBeLessThan(changelog.indexOf('## [1.36.0]'));
    const current = changelog.slice(changelog.indexOf('## [1.36.1]'), changelog.indexOf('## [1.36.0]'));
    // A real words for the symptom, so it is recognisable if it ever returns.
    expect(current).toMatch(/Asking GitHub/);
    expect(current).toMatch(/until the page was reloaded by hand/i);
    // The cause, not just the symptom: the button waited on a whole-page revalidation.
    expect(current).toMatch(/revalidated the entire Settings page/i);
    expect(current).toMatch(/No migration/i);
  });
  ...
  it('MUST-7.1: the 1.35.0 release is still recorded intact (append-only discipline)', () => {
    ...
    // The setting what was asked for, in the shape he asked for it.
  ...
  it('MUST-7.1: the 1.34.0 release is still recorded intact (append-only discipline)', () => {
    ...
    // The two behaviours what was asked for, in the words he used for them.
  ...
  it('MUST-7.1: the 1.33.0 release is still recorded intact (append-only discipline)', () => {
    ...
    // The release's headline: the reported problem, and the shape that replaced it.
    ...
    // The button that did nothing, in the words the report used for it.
