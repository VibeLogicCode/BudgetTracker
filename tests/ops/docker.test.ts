...
describe('version and changelog', () => {
  ...
  it('MUST-7.1: the 1.47.0 release', () => {
    const pkg = JSON.parse(read('package.json')) as { version: string };
    expect(pkg.version).toBe('1.47.0');
    const changelog = read('CHANGELOG.md');
    expect(changelog).toMatch(/^## \[1\.47\.0\] - 2026-09-18$/m);
    expect(changelog.indexOf('## Unreleased')).toBeLessThan(changelog.indexOf('## [1.47.0]'));
    expect(changelog.indexOf('## [1.47.0]')).toBeLessThan(changelog.indexOf('## [1.46.0]'));
    const current = changelog.slice(changelog.indexOf('## [1.47.0]'), changelog.indexOf('## [1.46.0]'));
    expect(current).toMatch(/Interest on loans/i);
    expect(current).toMatch(/Reconcile to a statement/i);
    expect(current).toMatch(/not estimates/i);
