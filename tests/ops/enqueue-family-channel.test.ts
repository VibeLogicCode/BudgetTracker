...
const ENQUEUE_SITES: EnqueueSite[] = [
  ...
  },
  {
    file: 'src/lib/notify/raise.ts',
    nth: 6,
    marker: "eventId: 'loan_paid_off'",
    recipient: 'member',
    householdDerivedBody: false,
    familyChannelOnly: 'absent',
    why: "one loan's name and direction, sent to that loan's owner and -- when familyChannelNeedsOwnPass says the room is the only subscriber -- to the room as its own row. There is no figure in the body at all, so a self-scoped recipient sees nothing they could not already see on the loan itself; the same loop carries both recipients, which is why this is classified household.",
  },
  {
    file: 'src/lib/notify/evaluate/loans.ts',
    nth: 1,
    marker: "eventId: 'loan_interest_posted'",
    recipient: 'member',
    householdDerivedBody: false,
    familyChannelOnly: 'absent',
    why: "what each loan just posted and the balance it left, grouped by the loan's own owner_user_id -- so a member's row names only their own loans, and the room's row names every loan that posted. Nothing crosses between the two, so there is no personal delivery to withhold.",
  },
  {
    file: 'src/lib/notify/evaluate/loans.ts',
    nth: 2,
    marker: "eventId: 'loan_payment_missed'",
    recipient: 'member',
    householdDerivedBody: false,
    familyChannelOnly: 'absent',
    why: 'loan names and the period that closed with nothing paid into it, taken from loansFor(userId) -- which narrows to the recipient\'s own loans for a member and widens to every loan only for the null household pass.',
  },
  {
    file: 'src/lib/notify/evaluate/loans.ts',
    nth: 3,
    marker: "eventId: 'loan_reconcile_due'",
    recipient: 'member',
    householdDerivedBody: false,
    familyChannelOnly: 'absent',
    why: 'loan names and the date of the last statement, over the same loansFor(userId) roster, which is already narrowed per recipient.',
  },
  {
    file: 'src/lib/notify/evaluate/goals.ts',
    nth: 1,
    marker: "eventId: 'goal_reached'",
    recipient: 'member',
    householdDerivedBody: false,
    familyChannelOnly: 'absent',
    why: "a goal's name and its target, read through viewerFor(userId) for a member and HOUSEHOLD_VIEWER for the room -- so listGoals has already applied the recipient's own visibility before anything is rendered.",
  },
  {
    file: 'src/lib/notify/evaluate/goals.ts',
    nth: 2,
    marker: "eventId: 'goal_off_pace'",
    recipient: 'member',
    householdDerivedBody: false,
    familyChannelOnly: 'absent',
    why: 'the required and average monthly figures for one goal, from the same viewer-scoped listGoals read.',
  },
  {
